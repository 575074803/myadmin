<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:60:"E:\wampserver\wamp64\www\myadmin/app/admin\view\ad\type.html";i:1515571477;}*/ ?>
<?php
use think\Db;
?>
<!DOCTYPE html>
            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
						<div class="">
                            <a href="<?php echo url('type_add'); ?>"  class="layui-btn layui-btn-sm">添加广告位</a>
                        </div>
                        <table class="layui-table" lay-size="sm">
						  ...
						  <tbody>
								...
								{
									$row=Db::name('adtype')->where('parentid',$id)->where('siteid',$siteid)->order('orderid asc')->select();
									foreach($row as $vo){
								?>
		...
		function AjaxClearAll()
		{
			...
			if(ckobj.size() > 0)
			{
				if(confirm("系统会自动删除类别下所有子类别以及信息，确定删除吗？"))
				{
					...
					$.post("<?php echo url('type_all_del');?>", {tablename:"<?php echo $tablename; ?>",ids:ids}, function (data) {
						if(data.status == 200){
							alert(data.msg);
							location=location;
						}
